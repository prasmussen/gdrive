...
func (self *Drive) prepareSyncRoot(args UploadSyncArgs) (*drive.File, error) {
	...
	f, err := self.service.Files.Get(args.RootId).Fields(fields...).SupportsAllDrives(true).Do()
	if err != nil {
	...
	f, err = self.service.Files.Update(f.Id, dstFile).Fields(fields...).SupportsAllDrives(true).Do()
	if err != nil {
...
func (self *Drive) uploadMissingFile(parentId string, lf *LocalFile, args UploadSyncArgs, try int) error {
	...
	_, err = self.service.Files.Create(dstFile).Fields("id", "name", "size", "md5Checksum").Context(ctx).Media(reader, chunkSize).SupportsAllDrives(true).Do()
	if err != nil {
...
func (self *Drive) updateChangedFile(cf *changedFile, args UploadSyncArgs, try int) error {
	...
	_, err = self.service.Files.Update(cf.remote.file.Id, dstFile).Context(ctx).Media(reader, chunkSize).SupportsAllDrives(true).Do()
	if err != nil {
		if isBackendOrRateLimitError(err) && try < MaxErrorRetries {
			exponentialBackoffSleep(try)
			try++
...
func (self *Drive) dirIsEmpty(id string) (bool, error) {
	...
	fileList, err := self.service.Files.List().Q(query).SupportsAllDrives(true).Do()
	if err != nil {
