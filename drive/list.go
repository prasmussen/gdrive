...
func (self *Drive) listAllFiles(args listAllFilesArgs) ([]*drive.File, error) {
	...
	err := self.service.Files.List().Q(args.query).Fields(args.fields...).Spaces("drive,photos").OrderBy(args.sortOrder).PageSize(pageSize).Pages(context.TODO(), func(fl *drive.FileList) error {
